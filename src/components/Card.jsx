...
const Card = (props) => {
    ...
    // };
    const {id,recipename,imageURL,description,ingredients,instructions}= props.recipe
    console.log(props.recipe)
  return (
    <Link
    to={`/details/${id}`}
    className="mr-[3.3%] mb-[3%] text-center w-[30%] bg-red-500 rounded-2xl  b p-5 shadow-[10px_10px_15px_0px_rgba(0,0,0,0.1)] hover:scale-[1.01] duration-200"
>
    <img className="w-[80%] ml-7" src={imageURL} alt={recipename} />
    <h1 className="mt-5 mb-3 text-xl text-semibold text-white">{recipename}</h1>
    ...
    <p className="flex justify-center mt-4">
            ...
            <br />
            <button className='bg-zinc-200 px-44  py-2 rounded-md text-black hover:text-white hover:bg-green-500 duration-200'>Share</button>
        </p>
</Link>
  )
}
